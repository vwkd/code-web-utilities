...
Deno.test("walk chain by link", () => {
    ...
    assertEquals(
        walkChainCall({ startNode: b1, linkName: "parent", callback: arrows }),
        "a1 <- b1"
    );
    assertEquals(
        walkChainCall({ startNode: b2, linkName: "parent", callback: arrows }),
        "a1 <- b2"
    );
    assertEquals(
        walkChainCall({ startNode: c1, linkName: "parent", callback: arrows }),
        "a1 <- b1 <- c1"
    );
    assertEquals(
        walkChainCall({ startNode: c2, linkName: "parent", callback: arrows }),
        "a1 <- b1 <- c2"
    );
});
